...
| **STT / polish endpoint** | The transcript the server returns is **typed into the focused window** as synthetic keystrokes, or pasted. `\n` is sent as a real Enter key press. Other control characters, bidi and zero-width characters are passed through unchanged, and there is no cap on length. | A byovox bug that types something the server did not send, or that lets the endpoint reach beyond the focused window (running a command itself, writing files). | That a malicious endpoint can type whatever it likes — including a newline that submits the line in a terminal or chat box. **You are trusting that server as much as you trust your own keyboard.** Point byovox only at servers you run or trust. |
