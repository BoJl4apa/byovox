...
- [ ] `inject.mode = "type"`, focus a chat box that sends on Enter, dictate "first foo, second bar": one *unsent* message reading `1. foo, 2. bar` (the comma is the model's stable output, measured 5/5 — #58) — nothing is submitted, no stray leading or trailing space
