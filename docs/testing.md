...
- [ ] `inject.mode = "type"`, focus a chat box that sends on Enter, dictate "first foo, second bar": one *unsent* message reading `1. foo, 2. bar` — the comma is rule 3's own printed example echoed back, not the model's choice; remove it from the example and the output loses it, 8/8 (#58) — nothing is submitted, no stray leading or trailing space
